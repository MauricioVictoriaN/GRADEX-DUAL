...
#     Multi-metric method selection: RMSE + R² + |BIAS| scoring.
#
...
#     - Technical note on GRADEX approximation for non-Gumbel distributions
#
# Input file:
...
LON_GAUGE        <- -75.58     # Target gauging point longitude (decimal degrees)
LAT_GAUGE        <-   6.30     # Target gauging point latitude  (decimal degrees)
...
compute_mk_statistics <- function(df) {
  if (nrow(df) < 5L)
    return(data.frame(tau = NA_real_, p_valor = NA_real_, pendiente = NA_real_))
  ...
  data.frame(tau      = round(as.numeric(mk$tau), 4),
             ...
             pendiente = round(as.numeric(mk$S),  3))
}
...
run_trend_analysis <- function(data) {
  ...
  message("Mann-Kendall results:"); print(trend_results)
  ...
  p <- ggplot2::ggplot(data, ggplot2::aes(x = anio, y = p_max,
    ...
    ggplot2::labs(title = "TREND ANALYSIS \u2014 ANNUAL MAXIMUM PRECIPITATION",
                  subtitle = "Mann-Kendall | Linear regression",
...
cv_metrics <- function(df, method, extra_info = NULL) {
  ...
  if (!is.null(extra_info)) {
    rows <- rbind(rows, data.frame(Metrica = names(extra_info),
                                   Valor   = unlist(extra_info),
                                   Metodo  = method,
                                   stringsAsFactors = FALSE))
  }
  rows
...
cv_scatter_plot <- function(df, metrics_df, method, subtitle_extra = "") {
  ...
  ggplot2::ggplot(df, ggplot2::aes(x = Observado, y = Predicho)) +
    ...
    ggplot2::geom_text(ggplot2::aes(label = Estacion),
...
run_cross_validation_both <- function(results, config) {
  ...
  reason <- if (selected_method == "Kriging" && score_ok > score_idw)
...
make_interp_map <- function(interp_df, stations_df, gauge_xy,
  ...
  p <- p +
    ...
    ggplot2::geom_text(
      ...
      nudge_x = 1200, nudge_y = -1200,
      size = 2.5, color = "white", fontface = "italic") +
    ggplot2::geom_point(
...
print_final_summary <- function(analysis_result, config, cv_result,
  ...
  if (!is.null(ir)) {
    ...
    cat(sprintf("\u2551   Independence (Ljung-Box):  %d/%d stations pass i.i.d. test%s\u2551\n",
                nrow(ir) - n_autocorr, nrow(ir),
                paste(rep(" ", max(0L, 20L - nchar(nrow(ir)))), collapse = "")))
  }
  if (!is.null(hr)) {
    n_disc <- sum(hr$discordancy$discordant, na.rm = TRUE)
    cat(sprintf("\u2551   Discordancy (D > %.1f):     %d station(s) flagged%s\u2551\n",
                DISC_THRESHOLD, n_disc,
                paste(rep(" ", 30L), collapse = "")))
    cat(sprintf("\u2551   Homogeneity (H-stat):      %-53s\u2551\n",
